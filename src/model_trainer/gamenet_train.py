...
def train(dataset, dataset_type, model_type, wandb_name):
    ...
    for epoch in range(EPOCH):
        ...
        test_results = eval(model, data_eval, voc_size)

        print ('training time: {}, test time: {}'.format(time.time() - tic,
            time.time() - tic2))

        train_results =  eval(model, data_train, voc_size)
...
def eval(model, data_eval, voc_size):
    ...
    patient_medicine_arr = np.zeros((len(data_eval), voc_size[2]))
    ...
    coverage = (len(covered_medicine)/voc_size[2]) * 100
